...
void led_init(rmt_channel_t chan, gpio_num_t pinNum)
{
	static const char *TAG = "led_init";
	ESP_LOGV(TAG,"led_init(%d) called", pinNum);
	rmt_config_t conf;
...
{
	return rmt_write_sample(channel, (const uint8_t*)src, num_pixels*3, false);
}
